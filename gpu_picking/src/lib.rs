...
impl GPUPicking {
    ...
    /// Encodes GPU commands for picking and initiates the async readback.
    /// Only runs if a pick is triggered and no other readback is in flight.
    pub fn pick(&mut self, queue: &wgpu::Queue, encoder: &mut wgpu::CommandEncoder) {
        ...
        if let Some(receiver) = self.pending_result_future.as_mut() {
            ...
            match receiver.try_recv() {
                Ok(Some(Ok(()))) => {
                    ...
                    let result_slice: &[u32] = bytemuck::from_bytes::<[u32; 257]>(&data);
                    let count = result_slice[0] as usize;

                    if count <= 256 {
                        self.last_result = Some(result_slice[1..=count].to_vec());
                    } else {
                        self.last_result = None; // Data is corrupt or invalid.
                    }
